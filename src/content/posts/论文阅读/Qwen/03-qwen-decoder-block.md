...
> 承接 [[论文阅读/qwen/02-架构解剖]]：02 把整机拆成五模块，其中 ② 24×DecoderLayer 的内部只给了结构示意。本篇把单层 Block 内部四个算子逐个拆开——每个算子替代了什么旧方案、为什么这么换、代价是什么。索引见 [[论文阅读/qwen/00-qwen阅读地图]]。
...
训练目标与损失函数见 [[论文阅读/qwen/04-qwen输入与训练]]。推理时的 KV Cache 与采样见 [[论文阅读/qwen/05-qwen推理与采样]]。config 到源码的映射见 [[论文阅读/qwen/06-qwen模型文件与代码]]。
