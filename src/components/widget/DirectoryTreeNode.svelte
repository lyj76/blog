...
function bindTreeToggle() {
	...
	toggleBound = true;
	const title = document.getElementById("reading-sidebar-title");
	if (!title) return;
	title.addEventListener("click", () => {
		const next = !get(treeCollapsed);
		treeCollapsed.set(next);
		document.getElementById("reading-sidebar-chevron")?.classList.toggle("rotate-180", next);
	});
}
    ...
    {:else}
        <div class="flex items-center w-full">
            <button
                ...
                on:mouseenter={(e) => showTooltip(e.currentTarget as HTMLElement, node.name)}
                on:mouseleave={hideTooltip}
                class="flex items-center gap-2 flex-1 min-w-0 py-1.5 rounded-lg transition-all duration-200 hover:bg-black/5 dark:hover:bg-white/10 text-neutral-800 dark:text-neutral-200 font-medium text-left"
                style="padding-left: {level * 0.75 + 0.5}rem"
            >
                <Icon
            ...
            {#if node.count !== undefined}
                {#if node.category}
                    <a
                        href={getCategoryUrl(node.category)}
                        title={`查看 ${node.category} 分类下的文章`}
                        class="text-xs px-1.5 py-0.5 rounded-full bg-black/5 dark:bg-white/10 text-neutral-500 dark:text-neutral-400 shrink-0 ml-1 transition hover:text-[var(--primary)] hover:bg-black/10 dark:hover:bg-white/20"
                    >{node.count}</a>
                {:else}
                    <span class="text-xs px-1.5 py-0.5 rounded-full bg-black/5 dark:bg-white/10 text-neutral-500 dark:text-neutral-400 shrink-0 ml-1">{node.count}</span>
                {/if}
            {/if}
        </div>
